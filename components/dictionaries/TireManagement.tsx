...
const tireSchema = z.object({
    ...
    quantity: z.number().min(1, 'Количество должно быть >= 1').default(1), // Bulk add
...
const TireManagement: React.FC = () => {
    ...
    const groupedData = useMemo(() => {
        ...
        tires.forEach(t => {
            // Grouping Key: Brand + Model + Size + Status + (Vehicle OR Storage) + InstallDate
            // This ensures tires mounted on same car same day are grouped.
            // Tires in stock are grouped by storage.
            ...
            group.count++;
            group.ids.push(t.id);
            // Sum mileage for averaging later
    ...
    const handleEdit = (groupId: string) => {
        ...
        // Find the actual first tire entity to populate fields
        const tire = tires.find(t => t.id === groupId);
        if (!tire) return;

        reset({ 
            ...tire, 
            quantity: group.count, // Show count of group
        });
        setIsModalOpen(true);
    };
    ...
    // Функция автозаполнения из номенклатуры
    const handleFillFromStock = (stockItemId: string) => {
        ...
        // Эвристика: пытаемся разобрать название "Шина Nokian Hakkapeliitta 205/55 R16"
        const parts = item.name.split(' ');
        let brand = '';
        let model = '';
        
        // Очень простая логика, которую пользователь сможет поправить руками
        if (parts.length > 1) brand = parts[0]; 
        if (parts.length > 2) model = parts.slice(1).join(' ');

        setValue('brand', brand);
        setValue('model', model); // Остальное в модель, пользователь поправит
        
        // Если есть цена
        if (item.lastPurchasePrice) setValue('purchasePrice', item.lastPurchasePrice);
        
        // Если номенклатура привязана к складу, подставим склад
    ...
    const onSubmit = async (data: TireFormData) => {
        try {
            // Logic: 
            // If ID exists, we are editing. Check if user wants to update ALL in group or split?
            // For simplicity: If editing a group, we update ALL tires in that group with the new props.
            
            // If new: Create N copies.
            
            if (data.id) {
                // Find group based on original ID
                const group = groupedData.find(g => g.ids.includes(data.id!));
                if (group) {
                    const updates = group.ids.map(tid => {
                        const payload: any = { ...data, id: tid };
                        delete payload.quantity;
                        // Keep distinct mileages? No, better keep them or they get reset?
                        // We merge data into existing objects basically
        ...
        <div>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 mb-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-gray-800 dark:text-gray-200 text-sm">Метод учета износа:</span>
                </div>
                <div className="flex gap-4">
                    <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        />
                        <span className="text-sm text-gray-700 dark:text-gray-300">По факту установки</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input 
                            type="radio" 
                            ...
                            className="h-4 w-4 text-blue-600"
                        />
                        <span className="text-sm text-gray-700 dark:text-gray-300">Строго по сезону</span>
                    </label>
                </div>
            </div>

            <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-semibold text-gray-800 dark:text-white">Учет шин</h3>
                <button onClick={handleAddNew} className="flex items-center gap-2 bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-blue-700 transition-colors"><PlusIcon className="h-5 w-5" /> Добавить</button>
            </div>
                ...
                <table className="w-full text-sm">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr className="text-left">
                            <th className="p-2 cursor-pointer" onClick={() => handleSort('brand')}>Бренд/Модель</th>
                            <th className="p-2">Размер</th>
                            <th className="p-2">Сезон</th>
                            <th className="p-2">Кол-во</th>
                            <th className="p-2">Статус</th>
                            <th className="p-2">Пробег (Общ/Зима/Лето)</th>
                            <th className="p-2">Ресурс</th>
                            <th className="p-2">Местоположение</th>
                    ...
                    <tbody>
                        {rows.map(g => {
                            ...
                            return (
                                <tr key={g.id} className="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <td className="p-2 font-medium">
                                        <div>{g.brand} {g.model}</div>
                                    </td>
                                    <td className="p-2">{g.size}</td>
                                    <td className="p-2">{g.season === 'Summer' ? 'Лето' : g.season === 'Winter' ? 'Зима' : 'Всесезон'}</td>
                                    <td className="p-2 font-bold">{g.count}</td>
                                    <td className="p-2">
                                        <span className={`px-2 py-1 rounded-full text-xs ${g.status === 'Mounted' ? 'bg-blue-100 text-blue-800' : g.status === 'InStock' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                                            {g.status === 'Mounted' ? 'Установлена' : g.status === 'InStock' ? 'На складе' : 'Списана'}
                                        </span>
                                    </td>
                                    <td className="p-2 text-xs">
                                        <div><b>{g.avgMileage}</b> км</div>
                                        <div className="text-gray-500">З: {g.winterMileage} / Л: {g.summerMileage}</div>
                                    </td>
                                    <td className="p-2 text-xs">
                                        <div>{g.estimatedLifespanKm} км</div>
                                        <div className={`font-bold ${color}`}>{percent}%</div>
                                    </td>
                                    <td className="p-2 text-xs">
                                        <div>{g.locationDisplay}</div>
                                        {g.installDate && <div className="text-gray-500">с {new Date(g.installDate).toLocaleDateString()}</div>}
                                    </td>
                                    <td className="p-2 flex gap-2">
                                        <button onClick={() => handleEdit(g.id)} className="text-blue-500 hover:bg-blue-100 p-1 rounded transition-colors"><PencilIcon className="h-5 w-5"/></button>
                                        <button onClick={() => handleDeleteRequest(g)} className="text-red-500 hover:bg-red-100 p-1 rounded transition-colors"><TrashIcon className="h-5 w-5"/></button>
                                    </td>
                                </tr>
                            );
                        })}
            ...
            <Modal isOpen={isModalOpen} onClose={handleCancel} title={currentItem?.id ? "Редактировать (группу)" : "Новая шина"} footer={<><button onClick={handleCancel}>Отмена</button><button onClick={handleSubmit(onSubmit)}>Сохранить</button></>}>
