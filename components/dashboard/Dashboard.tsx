...
import { getDashboardData, getIssues, getVehicles, getAppSettings } from '../../services/mockApi';
...
import { KpiData, Vehicle, AppSettings } from '../../types';
...
const Dashboard: React.FC<DashboardProps> = ({ onNavigateToWaybill }) => {
    ...
    const [filters, setFilters] = useState(() => {
        ...
        // Default to last 6 months if no saved filters
        const today = new Date();
    ...
    useEffect(() => {
        Promise.all([getVehicles(), getAppSettings()]).then(([v, s]) => {
            setVehicles(v);
            setSettings(s);
        });
        fetchData(filters);
        ...
        <div className="space-y-6">
            ...
            {widgets.showFleetStats && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-6">
                    ...
                    <KpiCard title="Расход (Квартал)" value={(kpi?.fuelQuarter ?? 0).toLocaleString('ru-RU')} icon={<UserGroupIcon className="h-6 w-6 text-white" />} color="bg-purple-500" unit="л" />
