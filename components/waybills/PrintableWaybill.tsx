...
const PrintableWaybill: FC<PrintableWaybillProps> = ({
  ...
  vehicle,
  driver,
  organization, // Added to destructuring
  dispatcher,
  controller,
  fuelType,
  allOrganizations,
  ...
  // Logic to determine effective organization fields (inheriting from parent if needed)
  const effectiveOrgFields = useMemo(() => {
    if (!organization) return {};
    
    // Если есть родительская организация, попробуем взять данные оттуда,
    // если в текущей (дочерней) они не заполнены или для "официального" бланка нужны реквизиты головы.
    // В данном случае, если это филиал (есть parentOrganizationId), берем данные головы.
    if (organization.parentOrganizationId) {
        const parent = allOrganizations.find(o => o.id === organization.parentOrganizationId);
        if (parent) {
            return {
                name: parent.fullName || parent.shortName, // Usually full name for documents
                address: parent.address || organization.address, // Fallback to child address if parent missing? Usually parent legal address.
  ...
  const renderValue = useCallback(
    (id: FieldKey) => {
      switch (id) {
        ...
        case 'orgName':
          return effectiveOrgFields.name ?? '';
        case 'orgAddress':
          return effectiveOrgFields.address ?? '';
        case 'orgInn':
          return effectiveOrgFields.inn ?? '';
        case 'validFromDay':
  ...
  const handleDragStart = useCallback(
    (id: FieldKey, e: MouseEvent<HTMLDivElement>) => {
  ...
  useEffect(() => {
    ...
    const onMove = (event: globalThis.MouseEvent) => handleDrag(event);
  ...
  return createPortal(
    ...
    >
      <ConfirmationModal
        ...
        message="Сбросить позиции и смещения к заводским настройкам?"
