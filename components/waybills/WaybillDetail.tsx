...
export const WaybillDetail: React.FC<WaybillDetailProps> = ({ waybill, isPrefill, onClose }) => {
  ...
  // Custom Correction Handler overlapping hook's logic because hook was a bit too simple
  const performCorrection = async (reason: string) => {
      if (!('id' in formData && formData.id)) return;
      // We import `changeWaybillStatus` from api directly to pass reason
      const { changeWaybillStatus } = await import('../../services/mockApi'); 
      try {
        ...
        // Invalidate queries to ensure list updates immediately
