...
const Diagnostics: React.FC = () => {
  ...
  // Danger zone
  const clearUnknownKeys = async () => {
    if (!window.confirm('Удалить все compat:unknown:* ключи?')) return;
  ...
  const clearAuditLog = async () => {
    if (!window.confirm('Удалить журнал аудита (индекс + чанки)? Это необратимо.')) return;
    setBusy('clearAudit');
    try {
      const idx = await loadJSON<any[]>(AUDIT_INDEX_KEY, []);
  ...
  const clearUiContext = async () => {
    if (!window.confirm('Сбросить UI-контекст (__ctx__:*)?')) return;
    setBusy('clearCtx');
    try {
      const keys = await storageKeys();
  ...
  const backupDb = async () => {
    if (!window.confirm('Сделать бэкап текущих данных (кроме служебных) в __backup_before_import__?')) return;
    setBusy('backup');
    try {
      const keys = await storageKeys();
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Danger zone */}
      <section className="border rounded-lg p-3 dark:border-gray-700">
        <div className="font-semibold mb-2 text-red-600">Опасная зона</div>
        <div className="text-xs text-gray-600 mb-3">Действия ниже необратимы. Используйте с осторожностью.</div>
        <div className="flex flex-wrap gap-2">
          <button className="px-3 py-1 rounded bg-red-600 text-white hover:bg-red-700 disabled:opacity-50"
            onClick={clearUnknownKeys} disabled={busy === 'clearUnknown'}>Удалить compat:unknown:* (все)</button>
          <button className="px-3 py-1 rounded bg-red-600 text-white hover:bg-red-700 disabled:opacity-50"
            onClick={clearAuditLog} disabled={busy === 'clearAudit'}>Очистить журнал аудита</button>
          <button className="px-3 py-1 rounded bg-red-600 text-white hover:bg-red-700 disabled:opacity-50"
            onClick={clearUiContext} disabled={busy === 'clearCtx'}>Сбросить UI-контекст (__ctx__:*)</button>
          <button className="px-3 py-1 rounded bg-gray-200 dark:bg-gray-700 disabled:opacity-50"
            onClick={backupDb} disabled={busy === 'backup'}>Сделать бэкап в {BACKUP_KEY}</button>
        </div>
      </section>
    </div>
  );
