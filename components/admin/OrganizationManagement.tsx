...
const organizationSchema = z.object({
    ...
    // Make isOwn optional to avoid conflicts between schema (strict bool) and form default values.
...
const OrganizationManagement = () => {
    ...
    const onSubmit = async (data: OrganizationFormData) => {
        try {
            // Ensure isOwn is defined (schema makes it optional)
            const safeData = { ...data, isOwn: data.isOwn ?? false };
            if (safeData.id) {
                await updateOrganization(safeData as Organization);
            } else {
                await addOrganization(safeData as Omit<Organization, 'id'>);
            }
