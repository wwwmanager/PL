...
import { 
    ...
    calculateDriverBalance, // Import new dynamic function
...
const emptyWaybill: Omit<Waybill, 'id'> = {
    ...
    reviewerComment: '',
    deviationReason: '',
    calculationMethod: 'by_total', // Default to new method
};
...
export const useWaybillForm = (
    ...
    const calculationStats = useMemo(() => {
        return calculateStats(
            ...
            formData.date,
            dayMode,
            formData.calculationMethod // Pass current method
        );
    ...
    }, [formData.driverId, formData.date, allTransactions]); // Re-run if transactions change (though queries handle refetch, useEffect deps need trigger)
    ...
    useEffect(() => {
        ...
        if (isPrefill && initialWaybill) {
            formDataToSet = {
                ...
                calculationMethod: initialWaybill.calculationMethod ?? 'by_total', // Inherit or default
            };
            ...
            if (driver) {
             ...
             // Ensure calculationMethod is set if editing older records
        ...
        setIsFuelPlannedManual(false); // Reset manual flag on new load
    ...
    useEffect(() => {
        ...
        // CHECK: Should we overwrite fuelPlanned?
        // Only if it's NOT manual OR if we are generating a new waybill (routes changed, etc)
        // BUT: For Imported Waybills (loaded from DB), fuelPlanned might differ from calculation.
        // We want to PRESERVE the DB value initially.
        // We want to UPDATE it only if the user changes something (routes, method, vehicle).
        
        // Helper to detect if routes changed from initial
        ...
        if (isFuelPlannedManual) {
            ...
            // Routes haven't changed from DB load. Keep DB value (even if it differs from calc).
            // This handles the Excel import case where imported '9' != calculated '9.4'.
            effectiveFuelPlanned = initialFormData.fuelPlanned;
        } else {
            // Routes changed OR it's a new waybill OR we are prefilling. Auto-calc.
        ...
        initialFormData,
        isPrefill,
        formData.routes // Implicitly in calculationStats but needed for logic check
    ]);
    ...
    const updateWaybillNumberForDriverInternal = async (driver: Employee | null, currentData: Omit<Waybill, 'id'> | Waybill) => {
        if (!driver?.id) {
            // Keep number empty if no driver, or reset if changed
    ...
    const handleVehicleChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        // Reset manual flag on vehicle change to trigger recalculation
    ...
    const handleMethodChange = (method: WaybillCalculationMethod) => {
        // Reset manual flag to allow re-calc based on new method
        setIsFuelPlannedManual(false);
    ...
    const handleAddRoute = () => {
        setIsFuelPlannedManual(false); // Reset to allow auto-calc for new route
    ...
    const handleRouteUpdate = (id: string, field: keyof Route, value: any) => {
        ...
        // If route details change, we should re-enable auto calculation
    ...
    const validateForm = async (): Promise<boolean> => {
        ...
        // Allow empty number for new records (auto-generation)
