...
import { useQuery, useMutation, useQueryClient, useInfiniteQuery, InfiniteData, QueryKey } from '@tanstack/react-query';
import {
  getWaybills,
  getVehicles,
  getEmployees,
  ...
  deleteWaybill,
  changeWaybillStatus,
  changeWaybillStatusBulk, // Added import
  getFuelTypes,
  getSavedRoutes,
  getSeasonSettings,
  getGarageStockItems,
...
export const useChangeWaybillStatus = () => {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      ...
      const updatedWaybill = result.data;
      if (updatedWaybill) {
          queryClient.setQueriesData({ queryKey: QUERY_KEYS.waybills }, (oldData: any) => {
              if (!oldData) return oldData;
              if (Array.isArray(oldData)) {
                  return oldData.map((w: Waybill) => w.id === updatedWaybill.id ? updatedWaybill : w);
              }
              return oldData;
          });
      }
